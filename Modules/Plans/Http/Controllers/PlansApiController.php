                ...
                if ($projectId) {
                    ...
                    if (Gate::allows("edit", [$project])) {
                        ...
                        if (!empty($request->input("plan"))) {
                            $plans = $plans->where(function ($query) use ($request) {
                                $query
    ...
    public function store(PlanStoreRequest $request)
    {
        try {
            ...
            if ($projectId) {
                ...
                if (Gate::allows("edit", [$project])) {
                    $requestData["price"] = number_format(
                        intval(preg_replace("/[^0-9]/", "", $requestData["price"])) / 100,
                        2,
                        ",",
                        "."
                    );
                        ...
                        if (!empty($plan)) {
                            ...
                            foreach ($request["products"] as $product) {
                                $productPlan->create([
                                    ...
                                    "currency_type_enum" => $productPlan
                                        ->present()
                                        ->getCurrency($product["currency_type_enum"]),
                                ]);
                            }
        ...
        try {
            ...
            if ($projectId) {
                ...
                if (Gate::allows("edit", [$project])) {
                    ...
                    if (
                        (count($plan->plansSales) > 0 && (!empty($resultId) || !empty($resultAmount))) ||
                        count($requestData["products"]) > count($productPlanId)
                    ) {
                        return response()->json(
                            ["message" => "Impossível editar os produtos do plano pois possui vendas associadas."],
                            400
                        );
                    }
                    ...
                    if (!empty($requestData["products"]) && !empty($requestData["product_amounts"])) {
                        foreach ($requestData["products"] as $keyProduct => $product) {
                            if (
                                empty($requestData["product_cost"][$keyProduct]) ||
                                $requestData["product_cost"][$keyProduct] == "0.00"
                            ) {
                                $requestData["product_cost"][$keyProduct] = 0;
                            } else {
                                $requestData["product_cost"][$keyProduct] = preg_replace(
                                    "/[^0-9]/",
                                    "",
                                    $requestData["product_cost"][$keyProduct]
                                );
                            }

                            $productPlan = ProductPlan::where("product_id", $requestData["products"][$keyProduct])
                            ...
                            if (!empty($productPlan)) {
                                ...
                                $productPlan->currency_type_enum = $productPlan
                                    ->present()
                                ...
                                $productPlan = ProductPlan::create([
                                    ...
                                    "currency_type_enum" => $objProductPlan
                                        ->present()
                                        ->getCurrency($requestData["currency"][$keyProduct]),
                                ]);
                                $idsProductPlan[] = $productPlan->id;
                    ...
                    if (count($idsProductPlan) > 0) {
                        $rowsProductPlan = ProductPlan::where("plan_id", $plan->id)
                            ->whereNotIn("id", $idsProductPlan)
                            ->get();
        ...
        try {
            ...
            if (!empty($plan)) {
                ...
                foreach ($requestData["products"] as $product) {
                    if (
                        !in_array(
                            current(Hashids::decode($product["id"])),
                            $productsPlan->pluck("product_id")->toArray()
                        )
                    ) {
                        $productPlanModel->create([
                            ...
                            "amount" => $product["amount"] ?? 1,
                            "cost" => $product["value"] ? preg_replace("/[^0-9]/", "", $product["value"]) : 0,
                            "currency_type_enum" => $productPlanModel
                                ->present()
                                ->getCurrency($product["currency_type_enum"]),
                        ]);
                    } else {
                        $productPlanModel
                            ->where("plan_id", $plan->id)
                            ->where("product_id", current(Hashids::decode($product["id"])))
                            ->update([
                                "amount" => $product["amount"] ?? 1,
                                "cost" => $product["value"] ? preg_replace("/[^0-9]/", "", $product["value"]) : 0,
                                "currency_type_enum" => $productPlanModel
        ...
        try {
            ...
            if (isset($id)) {
                ...
                if ($planId) {
                    //hash Ok
                    $plan = $planModel
                        ->with(["productsPlans", "plansSales", "project", "affiliateLinks"])
                        ->where("id", $planId)
                        ->first();
                    ...
                    if (Gate::allows("edit", [$project])) {
                        if (count($plan->plansSales) > 0) {
                            return response()->json(
                                ["message" => "Impossível excluir, possui vendas associadas a este plano."],
                                400
                            );
                        }
                ...
                return response()->json(
                    ["message" => "Impossível excluir, ocorreu um erro ao buscar dados do plano."],
                    400
                );
            }
        } catch (Exception $e) {
    ...
    public function updateBulkCost(Request $request)
    {
        try {
            ...
            if ($updateAllCurrency == 1) {
                ProductPlan::whereIn("plan_id", $project->plans->pluck("id")->toArray())->update([
    ...
    public function saveConfigCustomProducts(Request $request)
    {
        ...
        if (count($request->input("productsPlan", [])) > 0) {
            ...
            foreach ($productsPlanIds as $productPlanId) {
                $details[$productPlanId]["type"] = !empty($request->type[$productPlanId])
                    ? $request->type[$productPlanId]
                    : [];
                $details[$productPlanId]["label"] = !empty($request->label[$productPlanId])
                    ? $request->label[$productPlanId]
                    : [];
            }
            ...
            foreach ($itens as $productPlanId => $config) {
                ...
                if (!empty($productPlan)) {
                    ...
                    if ($allow_change_in_block === true) {
                        ...
                        $this->updateAllConfigCustomProduct(
                            $productShopfyId->shopify_id,
                            $config,
                            !empty($request->is_custom[$productPlanId]) ? 1 : 0
                        );
                    }
            ...
            $productPlans = ProductPlan::where("plan_id", current(Hashids::decode($request->plan)))
                ->where("product_id", current(Hashids::decode($request->product_id)))
                ->get();
            foreach ($productPlans as $productPlan) {
            ...
            if ($allow_change_in_block === true) {
                ...
                $this->updateAllConfigCustomProduct(
                    $productShopfyId->shopify_id,
                    [],
                    !empty($request->is_custom[$productPlan->id]) ? 1 : 0
                );
            }
        }
