...
class GetNetStatementService
{
    ...
    public function performWebStatement(stdClass $data, array $filters = [], $limit = false)
    {
        ...
        $items = collect($this->statementItems)
            ->sortByDesc("sequence")
            ->values();
    ...
    private function preparesNodeTransactions($transactions): void
    {
        foreach ($transactions as $item) {
            ...
            if (
                ...
                $paymentDateNumeric = $details->payment_date
                    ? Carbon::parse($details->payment_date)->format("Ymd")
                    : null;
                ...
                foreach (
                    ["paymentDate", "transactionDate", "subSellerRateClosingDate", "subSellerRateConfirmDate"]
                    as $date
                ) {
                    if ($date) {
                ...
                if ($hasOrderId && $this->isDigitalProduct($summary->order_id)) {
                    ...
                    $hasValidTracking =
                        (bool) Redis::connection("redis-statement")->get(
                            "sale:has:tracking:{$orderFromGetNetOrderId->getSaleId()}"
                        ) ?? true;
                }
                ...
                if (
                    $hasOrderId &&
                    !$isTransactionCredit &&
                    $transactionStatusCode == self::TRANSACTION_STATUS_CODE_APROVADO
                ) {
                    $details
                        ->setStatus("Estornado")
                    ...
                    $details
                        ->setStatus("Aguardando postagem válida")
                        ->setDescription($this->formatDate($summary->transaction_date))
                    ...
                    ($hasOrderId &&
                        ...
                        $hasValidTracking &&
                        $isReleaseStatus &&
                        empty($subSellerRateConfirmDate)) ||
                    ($transactionStatusCode == self::TRANSACTION_STATUS_CODE_ESTORNADA &&
                        empty($subSellerRateConfirmDate))
                ) {
                    $details
                        ->setStatus("Aguardando liquidação")
                        ->setDescription($this->formatDate($summary->transaction_date))
                    ...
                    ($hasOrderId &&
                        ...
                        in_array($transactionStatusCode, [
                            self::TRANSACTION_STATUS_CODE_APROVADO,
                ...
                if ($this->canAddStatementItem($date, $details->getType(), $paidWith)) {
                    ...
                    $statementItem->isInvite = $amount <= 5.0;
                    ...
                    $statementItem->sequence = $statementItem->date
                        ? Carbon::createFromFormat("d/m/Y", $statementItem->date)->format("Ymd")
                        : 0;
    ...
    public function setOrderFromGetNetOrderId($getOrderId): ?Order
    {
        if (!empty($getOrderId)) {
            ...
            if (count($arrayOrderId)) {
                ...
                $order = new Order();
                return $order
                    ->setSaleId(current(Hashids::connection("sale_id")->decode($arrayOrderId[0])))
                    ->setHashId($hashId)
    ...
    private function preparesNodeAdjustments($adjustments): void
    {
        foreach ($adjustments as $adjustment) {
            if ($adjustment->cnpj_marketplace != $adjustment->cpfcnpj_subseller) {
                ...
                foreach (
                    ["paymentDate", "adjustmentDate", "subSellerRateClosingDate", "subSellerRateConfirmDate"]
                    as $date
                ) {
                    if ($date) {
                        ${$date} = $this->formatDate(${$date});
                ...
                $details = new Details();
                $details
                    ->setStatus("Ajuste de " . ($adjustment->transaction_sign == "+" ? "crédito" : "débito"))
                    ->setDescription($adjustment->adjustment_reason)
                    ->setType(
                        $adjustment->transaction_sign == "+"
                            ? Details::STATUS_ADJUSTMENT_CREDIT
                            : Details::STATUS_ADJUSTMENT_DEBIT
                    );
                ...
                if ($this->canAddStatementItem($date, $details->getType(), null)) {
                    ...
                    if ($adjustIsChargeBack !== false || $adjustIsReverse !== false) {
                        ...
                        if (count($hashId) == 2) {
                            ...
                            $order = new Order();
                            $order
                                ->setSaleId(current(Hashids::connection("sale_id")->decode($hashId[1])))
                                ->setHashId($hashId[1])
                                ->setOrderId("-");
    ...
    private function preparesDatabasePixWithSaleSearch(): void
    {
        ...
        $pixSales = Sale::select(
            "transactions.value as transaction_value",
            "transactions.status as transaction_status",
            "transactions.release_date",
            "transactions.withdrawal_id",
            "sales.start_date",
            "sales.end_date",
            "sales.has_valid_tracking",
            "sales.id",
            "sales.delivery_id",
            "transactions.gateway_released_at"
        )
        ...
        foreach ($pixSales as $pix_sale) {
            ...
            $statementItem->transactionDate = $pix_sale->end_date
                ? Carbon::parse($pix_sale->end_date)->format("d/m/Y")
                : "";
    ...
    private function preparesDatabasePendingDebtsWithSaleSearch(): void
    {
        ...
        if (array_key_exists("sale_id", $this->filters) && !empty($this->filters["sale_id"])) {
            ...
            $pendingDebts = PendingDebt::select("pending_debts.*")
                ->join("pending_debt_withdrawals", function ($j) use ($withdrawal_id) {
                    return $j
                        ->on("pending_debt_withdrawals.pending_debt_id", "=", "pending_debts.id")
        ...
        foreach ($pendingDebts as $pendingDebt) {
            ...
            $paymentDate = $pendingDebt->payment_date
                ? Carbon::createFromFormat("Y-m-d", $pendingDebt->payment_date)->format("d/m/Y")
                : "";
            $transactionDate = $pendingDebt->request_date
                ? Carbon::createFromFormat("Y-m-d H:i:s", $pendingDebt->request_date)->format("d/m/Y")
                : "";
            $subSellerRateConfirmDate = $pendingDebt->confirm_date
                ? Carbon::createFromFormat("Y-m-d", $pendingDebt->confirm_date)->format("d/m/Y")
                : "";
            $sequence = $pendingDebt->payment_date
                ? Carbon::createFromFormat("Y-m-d", $pendingDebt->payment_date)->format("Ymd")
                : 0;
    ...
    public function performStatement(stdClass $data, array $filters = [])
    {
        if (isset($data->errors)) {
            ...
            $exception = new Exception(
                "Houve um erro ao processar a requisição na getnet em " .
                    __METHOD__ .
                    " :: " .
    ...
    public function getFiltersAndStatement($companyId)
    {
        ...
        $getNetBackOfficeService
            ->setStatementSubSellerId($credential->gateway_subseller_id)
