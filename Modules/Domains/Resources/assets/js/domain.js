...
$(document).ready(function () {
    ...
    function updateDomains(link = null) {
        ...
        $.ajax({
            ...
            success: function success(response) {
                ...
                if (response.data == "") {
                    $("#domain-table-body").html(
                        "<tr class='text-center'><td colspan='4' style='height: 70px; vertical-align: middle;'>Nenhum domínio encontrado</td></tr>"
                    );
                } else {
        ...
        $.ajax({
            ...
            success: function success(response) {
                ...
                $(".btn-update").on("click", function () {
                    ...
                    $("#btn-modal").hide();
                    var tbl = $("#new_registers_table tr")
                        .map(function (rowIdx, row) {
                            if (rowIdx > 0 && $(row).attr("data-save") == 0) {
                                var rowObj = $(row)
                                    .find("td")
                                    .map(function (cellIdx, cell) {
                                        var retVal = {};
    ...
    function modalUpdateDomains(index, value) {
        ...
        dados +=
            ...
            statusDomain[value.status] +
            '">' +
            value.status_translated +
            "</span></td>";
        dados += "<td style='text-align:center;'>";
        dados +=
            "<a role='button' class='mg-responsive details-domain pointer'       status='" +
            value.status +
            "' domain='" +
            value.id +
    ...
    function modalVerify(responseDomain) {
        ...
        $.each(responseDomain.data["zones"], function (index, value) {
            $("#tableDomainBody").append(
                "<tr>" + '<td class="table-title"><b>Novo servidor DNS :</b></td>' + "<td>" + value + "</td>" + "</tr>"
            );
        });
