...
class DomainRecordsApiController extends Controller
{
    public function index($projectId, $domainId)
    {
        try {
            ...
            activity()
                ->on($domainModel)
                ->tap(function (Activity $activity) use ($domainId) {
            ...
            $domain = $domainModel
                ->with([
                    "domainsRecords" => function ($query) use ($domainProject) {
                        $query
                            ->orWhere(function ($queryWhere) use ($domainProject) {
            ...
            if (
                ...
                if (
                    $domain->domainsRecords
                        ->where("type", $requestData["type-register"])
                        ->where("name", $subdomain)
                        ->where("content", $requestData["type-register"])
                        ->count() == 0
                ) {
                    // não existe record
    ...
    public function update(Request $request, $domain, $domainRecord): JsonResponse
    {
        try {
            ...
            $proxy = true;
            if (
                ($domainRecord->type == "A" || $domainRecord->type == "MX" || $domainRecord->type == "TXT") &&
                $request->input("proxy") != "1"
            ) {
                $proxy = false;
            }

            if (
                $domainRecord->type == "CNAME" &&
                $domainRecord->name === "www" &&
                $domainRecord->content == "shops.myshopify.com"
            ) {
                $proxy = false;
            }
