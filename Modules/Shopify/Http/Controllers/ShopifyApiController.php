...
class ShopifyApiController extends Controller
{
        ...
        try {
            ...
            try {
                ...
                if (empty($shopifyService->getClient())) {
                    return response()->json(['message' => 'Dados do shopify inválidos, revise os dados informados'],
                        400);
                }
            } catch (Exception $e) {
                report($e);
                ...
                if (!empty($shippingModel)) {
                    ...
                    if (!empty($shopifyIntegration)) {
                        ...
                        if (!empty($userProjectModel)) {
                            ...
                            return response()->json(['message' => 'Problema ao criar integração, tente novamente mais tarde'],
                                400);
                        }
                    } else {
                        $shippingModel->delete();
            ...
            return response()->json(['message' => 'Integração em andamento. Assim que tudo estiver pronto você será avisado(a)!'],
                200);
        } catch (Exception $e) {
                    ...
                    try {
                        ...
                        foreach ($project->shopifyIntegrations as $shopifyIntegration) {
                            ...
                            if (!empty($shopifyIntegration->theme_html)) {
                                $shopify->setTemplateHtml($shopifyIntegration->theme_file,
                                    $shopifyIntegration->theme_html);
                            }
                            if (!empty($shopifyIntegration->layout_theme_html)) {
                                $shopify->setTemplateHtml('layout/theme.liquid',
                        ...
                        //throwl
                        return response()->json(['message' => 'Problema ao desfazer integração, tente novamente mais tarde'],
                            Response::HTTP_BAD_REQUEST);
                    }
                } else {
                    return response()->json(['message' => 'Este projeto não tem integração com o shopify'],
                        Response::HTTP_BAD_REQUEST);
                }
            } else {
                //problema no id
                ...
                if (!empty($domain)) {
                    ...
                    if (!empty($project->shopify_id)) {
                        ...
                        try {
                            ...
                            foreach ($project->shopifyIntegrations as $shopifyIntegration) {
                                $shopify = new ShopifyService($shopifyIntegration->url_store,
                            ...
                            return response()->json(['message' => 'Integração com o shopify refeita'],
                                Response::HTTP_OK);
                        } catch (Exception $e) {
                            //throwl
                            return response()->json(['message' => 'Problema ao refazer integração, tente novamente mais tarde'],
                                Response::HTTP_BAD_REQUEST);
                        }
                    } else {
                        return response()->json(['message' => 'Este projeto não tem integração com o shopify'],
                    ...
                    return response()->json(['message' => 'Produtos do shopify importados, adicione um domínio para finalizar a sua integração'],
                        Response::HTTP_OK);
                }
            } else {
                //problema no id
        ...
        try {
            ...
            if (!empty($projectId)) {
                ...
                if (!empty($shopifyIntegration)) {
                    ...
                    return response()->json(['message' => 'Os Produtos do shopify estão sendo sincronizados.'],
                        Response::HTTP_OK);
                } else {
                    return response()->json(['message' => 'Problema ao sincronizar produtos, tente novamente mais tarde'],
                        Response::HTTP_BAD_REQUEST);
                }
            } else {
                return response()->json(['message' => 'Problema ao sincronizar produtos, tente novamente mais tarde'],
            ...
            return response()->json(['message' => 'Problema ao sincronizar produtos do shopify, tente novamente mais tarde'],
                Response::HTTP_BAD_REQUEST);
        }
    }
    ...
    public function synchronizeTemplates(Request $request)
    {
        try {
                ...
                if (!empty($domain)) {
                    if (!empty($project->shopify_id)) {
                        try {
                            ...
                            return response()->json(['message' => 'Sincronização do template com o shopify concluida com sucesso!'],
                                Response::HTTP_OK);
                        } catch (Exception $e) {
                            //throwl
                            return response()->json(['message' => 'Problema ao refazer integração, tente novamente mais tarde'],
                    ...
                    return response()->json(['message' => 'Você não tem nenhum domínio configurado'],
                        Response::HTTP_BAD_REQUEST);
                }
            } else {
                return response()->json(['message' => 'Projeto não encontrado'], Response::HTTP_BAD_REQUEST);
            ...
            return response()->json(['message' => 'Problema ao sincronizar template do shopify, tente novamente mais tarde'],
                Response::HTTP_BAD_REQUEST);
        }
    }
        ...
        if (!empty($data['project_id']) && isset($data['skip_to_cart'])) {
            ...
            if ($projectId) {
                ...
                try {
                    ...
                    return response()->json(['message' => 'Ocorreu um erro ao atualizar o skip to cart do projeto'],
                        400);
                }
            } else {
