...
class ExtractReportExportGateway implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents, WithMapping
{
    ...
    public function map($row): array
    {
        ...
        return [
            ...
            'is_owner' => ($transfer->transaction_type == Transaction::TYPE_PRODUCER || is_null($transfer->transaction_type)) ? 'SIM' : 'NAO',
            ...
            'sale_date' => !empty($transfer->transaction) ? Carbon::parse($transfer->transaction->sale->start_date)->format('d/m/Y') : '',
            'flag' => !empty($sale->flag) ? $sale->flag : $sale->present()->getPaymentFlag(),
            'status_name' => $sale->present()->getStatus($sale->status),
            'cashback_value' => $sale->payment_method <> 4 ? FoxUtils::formatMoney($sale->cashback->value / 100) : FoxUtils::formatMoney(0) ,
    ...
    {
        if ($type_enum == 1) return 'Entrada';
        if ($type_enum == 2) return 'Saída';
    }
}
