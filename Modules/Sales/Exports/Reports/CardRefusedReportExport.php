...
class CardRefusedReportExport implements FromQuery, WithHeadings, ShouldAutoSize, WithEvents, WithMapping
{
    ...
    public function query()
    {
        ...
        $salesExpired = Sale::
            ...
            })->whereIn('sales.status', [3])->where([['sales.payment_method', Sale::CREDIT_CARD_PAYMENT],])->with([
                'project',
                'customer',
        ...
        if (!empty($this->filters['client'])) {
            $customerSearch = Customer::where('name', 'like', '%' . $this->filters['client'] . '%')->pluck('id')->toArray();
