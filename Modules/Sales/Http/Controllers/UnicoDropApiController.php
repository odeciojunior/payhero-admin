...
use Modules\Sales\Transformers\SalesUnicodropExternalResource;
...
class UnicoDropApiController extends Controller
{
    public function index()
    {
        try {
            ...
            if (!empty($user)) {
                ...
                return SalesUnicodropExternalResource::collection($sales);
            } else {
    ...
    public function show($saleId)
    {
        try {
            ...
            if (!empty($user)) {
                $saleId = current(
                    Hashids::connection("sale_id")->decode($saleId)
                );
                $sale = $salesModel
                    ->with([
                        "transactions",
                ...
                if (!empty($sale)) {
                    ...
                    $sale->products = $products;
                    return new SalesUnicodropExternalResource($sale);
                } else {
                    return response()->json(
