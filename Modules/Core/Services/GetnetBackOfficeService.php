...
class GetnetBackOfficeService extends GetnetBaseService
{
    ...
    protected ?string $sellerId,
        $statementSubSellerId = null;
    protected Carbon $statementStartDate, $statementEndDate;
    protected ?string $statementDateField,
        $statementSaleHashId = "";
    ...
    public function getStatement($orderId = null)
    {
        if (empty($this->getStatementDateField()) && empty($this->getStatementSaleHashId())) {
            throw new LogicException(
                "É obrigatório especificar um campo de data para a busca quando não é enviado um OrderId"
            );
        } elseif (
            !empty($this->getStatementDateField()) &&
            !in_array($this->getStatementDateField(), [
                self::STATEMENT_DATE_SCHEDULE,
                self::STATEMENT_DATE_LIQUIDATION,
                self::STATEMENT_DATE_TRANSACTION,
            ])
        ) {
            throw new LogicException(
                'O campo de data para a busca deve ser "' .
                    self::STATEMENT_DATE_SCHEDULE .
                    '", "' .
                    self::STATEMENT_DATE_LIQUIDATION .
                    '" ou "' .
