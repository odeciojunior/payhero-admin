...
class ChargebackService
{
    function getQuery($filters)
    {
        ...
        if (request()->has("from_contestation") || request("date_type") == "expiration_date") {
            $getnetChargebacks->when(request("date_type"), function ($query, $search) {
                return $query->whereHas("sale.contestations", function ($query) use ($search) {
                    $dateRange = FoxUtils::validateDateRange(request("date_range"));

                    $query->whereBetween($search, [$dateRange[0] . " 00:00:00", $dateRange[1] . " 23:59:59"]);
                });
            });
            ...
            $getnetChargebacks->whereBetween($filters["date_type"], [
        ...
        if (!empty($filters["fantasy_name"])) {
            $getnetChargebacks->whereHas("company", function ($query) use ($filters) {
        ...
        if (!empty($filters["customer"])) {
            $getnetChargebacks->whereHas("sale", function ($query) use ($filters) {
        ...
        if (!empty($filters["customer_document"])) {
            ...
            $getnetChargebacks->whereHas("sale.customer", function ($query) use ($document) {
        ...
        if (!empty($filters["contestation_situation"])) {
            ...
            $getnetChargebacks->whereHas("sale.contestations", function ($query) use ($status) {
    ...
    public function getTotalApprovedSales($filters)
    {
        ...
        if ($filters["date_type"] == "transaction_date") {
            $totalSaleApproved->whereBetween("start_date", [$dateRange[0] . " 00:00:00", $dateRange[1] . " 23:59:59"]);
        } else {
            $totalSaleApproved->whereHas("getnetChargebacks", function ($query) use ($dateRange) {
                $query->whereBetween("adjustment_date", [$dateRange[0] . " 00:00:00", $dateRange[1] . " 23:59:59"]);
            });
        }
        ...
        if (!empty($filters["fantasy_name"])) {
            $totalSaleApproved->whereHas("user.companies", function ($query) use ($filters) {
                $query->where("fantasy_name", "like", "%" . $filters["fantasy_name"] . "%");
            });
        }
