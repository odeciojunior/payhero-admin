...
class ContestationService
{
    ...
    function getQuery($filters)
    {
        ...
        $contestations = SaleContestation::select(
            ...
            ->selectRaw(
                \DB::raw(
                    "(CASE WHEN expiration_date > '" .
                        Carbon::now()
                            ->addDay(2)
                            ->endOfDay() .
                        "' THEN 1 ELSE 0 END) as custom_expired"
                )
            )
        ...
        $contestations->when(request("date_type"), function ($query, $search) {
            ...
            return $query->whereBetween($search_input_date, [$dateRange[0] . " 00:00:00", $dateRange[1] . " 23:59:59"]);
        });
        ...
        $contestations->when(request("customer_document"), function ($query, $search) {
            return $query->where("customers.document", "like", "%" . $search . "%");
        });
    ...
    public function getWonContestationsTax($totalContestations, $totalWonContestations)
    {
        ...
        $totalContestationsTax =
            $totalWonContestations > 0
                ? number_format(($totalWonContestations * 100) / $totalContestations, 2, ",", ".") . "%"
                : "0,00%";
    ...
    public function getTotalApprovedSales($filters)
    {
        ...
        if (!empty($filters["transaction"])) {
            ...
            $transactions = array_map(function ($item) {
                return is_numeric($item) ? $item : current(Hashids::connection("sale_id")->decode($item));
            }, current($matches));
        ...
        if (!empty($filters["customer_document"])) {
            ...
            $totalSaleApproved->whereHas("customer", function ($query) use ($document) {
    ...
    public function getChargebackTax($totalChargebacks, $totalApprovedSales)
    {
        ...
        $totalChargebackTax =
            $totalChargebacks > 0
                ? number_format(($totalChargebacks * 100) / $totalApprovedSales, 2, ",", ".") . "%"
                : "0,00%";
    ...
    public function generateDispute(SaleContestation $contestation)
    {
        ...
        $sale = Sale::with(["transactions", "customer", "delivery", "trackings"])->find($contestation->sale_id);
        ...
        if (!empty($sale->affiliate_id)) {
            ...
            if (!empty($affiliateTransaction)) {
                ...
                $affiliateComission =
                    ($affiliateTransaction->currency == "dolar" ? 'US$ ' : 'R$ ') .
                    substr_replace($affiliateValue, ",", strlen($affiliateValue) - 2, 0);
            }
        ...
        foreach ($saleTrackings as $saleTracking) {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, "https://tracking.azcend.com.br/api/tracking/detail/" . $saleTracking);
        ...
        $pdf = PDF::loadView(
            "chargebacks::contestchargeback",
            compact("dataSale", "contestation", "products", "tracking", "sale_details")
        );
    ...
    private function uploadPdfToS3($pdf, $pdf_name)
    {
        ...
        Storage::disk("s3_documents")->put($path, $pdf->output(), "private");

        $path = Storage::disk("s3_documents")->temporaryUrl($path, Carbon::now()->addSeconds(60));
    ...
    public function sendContestationFiles($files)
    {
        ...
        foreach ($files["files"] as $file) {
            ...
            $pathamz = $amazonFileService->uploadFile($path, $file, $image_name, null, "private");
