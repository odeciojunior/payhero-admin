...
class SalesRecoveryService
{
    ...
    public function getSaleExpiredOrRefused(
        ...
        $salesExpired = Sale::select(
            ...
            DB::raw(
                "IF(products_sales_api.id IS NOT NULL, (products_sales_api.price * products_sales_api.quantity) / 100, (plan_sale.amount * plan_sale.plan_value)) AS value",
            ),
        )
        ...
        if (!in_array("all", $projectIds)) {
            ...
            if ($company_id == null) {
                ...
                $userId = Company::find($company_id)->user_id;
            }
