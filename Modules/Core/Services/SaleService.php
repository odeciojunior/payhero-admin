    ...
    public function getDetails($sale, $userCompanies)
    {
        ...
        if (!empty($sale->affiliate_id)) {
            ...
            if (!empty($affiliateTransaction)) {
                ...
                $affiliateComission = ($affiliateTransaction->currency == 'dolar' ? 'US$ ' : 'R$ ') . substr_replace(
                        $affiliateValue,
                        ',',
                        strlen($affiliateValue) - 2,
                        0
                    );
            }
        }
        ...
        if (preg_replace("/[^0-9]/", "", $sale->installment_tax_value) > 0) {
            $taxaReal = $totalToCalcTaxReal - preg_replace('/[^0-9]/', '', $comission) - preg_replace(
                    "/[^0-9]/",
                    "",
                        ...
                        $f1->where('sales.status', $salesModel->present()->getStatus('in_dispute'))
                            ->orWhere(
                                ...
                                function ($queryTracking) {
                                    ...
                                    $queryTracking->where(
                                        'system_status_enum',
                                        $presenter->getSystemStatusEnum('duplicated')
                                    );
                                }
                            );
                    }
