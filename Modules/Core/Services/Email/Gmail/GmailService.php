...
class GmailService
{
    ...
    public function syncContestations()
    {
        ...
        foreach ($emailMessages as $emailMessage) {
            ...
            if (!empty($emailMessage)) {
                ...
                $sale = Sale::whereHas("saleGatewayRequests", function ($query) use ($nsuTerminal) {
                    $query->where("gateway_result->credit->terminal_nsu", "=", $nsuTerminal);
                })->get();

                if (count($sale) == 0) {
                    report(
                        new Exception(
                            "Venda não encontrada nos emails de contestação com o nsu terminal = " . $nsuTerminal
                        )
                    );
                    continue;
                }
                if (count($sale) > 1) {
                    report(
                        new Exception(
                            "Mais de uma venda encontrada nos emails de contestação com o nsu terminal = " .
                                $nsuTerminal
                        )
                    );
                    continue;
    ...
    {
        $emailMessages = $this->getMessagesWithQuery(
            50,
            "from:(monitoria_marketplace@getnet.com.br) Valor do Chargeback",
            true
        );
    ...
    public function getFilesFromEmail($limit = 200, $recursive = false)
    {
        ...
        $get_day_latest_import = SaleContestation::select("file_date")
        ...
        $maxResults = $limit;
        $pageToken = null;
        $q = "from:(monitoria_marketplace@getnet.com.br) [MGM GETNET] E-Request azcend after:" . $date_after;
        $labelIds = ["INBOX"]; //CHAT - SENT - INBOX - IMPORTANT - TRASH - DRAFT - SPAM - CATEGORY_FORUMS - CATEGORY_UPDATES - CATEGORY_PERSONAL - CATEGORY_PROMOTIONS - CATEGORY_SOCIAL - STARRED - UNREAD
        $includeSpamTrash = false;
        $alt = "json";
