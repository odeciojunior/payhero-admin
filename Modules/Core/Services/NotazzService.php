...
class NotazzService
{
            ...
            if ($products) {
                ...
                $fields = json_encode(
                    [
                        ...
                        'DOCUMENT_DESCRIPTION' => 'Prestação de Serviço em intermediação de compra, desconsiderando outros custos',
                        //Descrição da nota fiscal (obrigatório somente para o método create_nfse e update_nfse)
                        'DOCUMENT_COMPETENCE' => ($sale->project->notazzIntegration->id == 5) ? Carbon::parse(
                            $sale->end_date
                        )
    ...
    public function generateRetroactiveInvoices()
    {
        try {
            ...
            $integrations = $notazzIntegration->with(['project', 'user'])
