    ...
    public function handle(TicketMessageEvent $event)
    {
        try {
            ...
            if ($ticketMessage->created_at >= $lastAdminMessageTime) {
                ...
                $domain = Domain::where('project_id', $project->id)->where('status', 3)->first();
                ...
                $sendGridService->sendEmail($fromEmail, $project->name, $customer->email, $customer->name, 'd-e44117575a044be193c2f7235bc22789', $data);
            }
        } catch (Exception $e) {
