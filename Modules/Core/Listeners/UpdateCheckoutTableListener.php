...
class UpdateCheckoutTableListener implements ShouldQueue
{
    ...
    public function handle($event)
    {
        ...
        $abandonedCarts = $checkoutModel
            ->select(
                "checkouts.id",
                "checkouts.created_at",
                "checkouts.project_id",
                "checkouts.status",
                "checkouts.email_sent_amount",
                "checkouts.sms_sent_amount",
                "logs.name",
                "logs.telephone"
            )
            ->leftjoin("logs", function ($join) {
                $join->on(
                    "logs.id",
                    "=",
                    DB::raw("(select max(logs.id) from logs WHERE logs.checkout_id = checkouts.id)")
                );
            })
            ->whereIn("status_enum", [
