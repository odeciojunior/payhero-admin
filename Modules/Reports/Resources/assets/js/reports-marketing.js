...
function loadOrigins(link = null) {
    ...
    link = `${resumeUrl}/origins?paginate=false&limit=all&date_range=${$("input[name='daterange']").val()}&origin=${$(
        "#origin"
        ...
        success: function success(response) {
            ...
            if (response.data.length == 0) {
                ...
                $("#block-origins").prepend(`
                    <footer class="footer-origins scroll-212" style="${
                        response.data.length > 10 ? "height: 510px; " : "height: 100%; "
...
function loadOperationalSystems() {
    ...
    $.ajax({
        ...
        url:
            mktUrl +
            "/operational-systems?project_id=" +
            $("#select_projects option:selected").val() +
            "&date_range=" +
            $("input[name='daterange']").val(),
        dataType: "json",
                                ...
                                <div class="box-payment-option pad-0">
                                    ...
                                    <div class="box-payment-option option">
                                        <div class="col-payment col-graph">
                                            <div class="bar blue" style="width:${
                                                data.percentage || "0$"
                                            }">barrinha</div>
                                        </div>
                                        <div class="col-payment">
                                            <span class="money-td green bold grey font-size-14 value-percent">${
                                                data.percentage
                                            }</span>
                                        </div>
                                    </div>
...
function loadBrazilMap() {
    ...
    $.ajax({
        ...
        url:
            mktUrl +
            "/sales-by-state?project_id=" +
            $("#select_projects option:selected").val() +
            "&date_range=" +
            $("input[name='daterange']").val() +
            "&map_filter=" +
            $("input[name='brazil_map_filter']:checked").val(),
        dataType: "json",
        headers: {
            Authorization: $('meta[name="access-token"]').attr("content"),
            Accept: "application/json",
