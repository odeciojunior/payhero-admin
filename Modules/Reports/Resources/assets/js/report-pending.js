...
$(document).ready(function () {
    ...
    function getAcquirer() {
        $.ajax({
            method: "GET",
            url: '/api/finances/acquirers/'+$('#companyTopbar').val(),
            dataType: "json",
            headers: {
                'Authorization': $('meta[name="access-token"]').attr('content'),
                'Accept': 'application/json',
    ...
    //$('.company_name').val( $("#companyTopbar option[value='"+sessionStorage.getItem('company_default')+"']").text() );
    $('.company_name').val( sessionStorage.getItem('company_default_name') );
    //$('.company_name').val( $("#companyTopbar").val() );
