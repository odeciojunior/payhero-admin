    ...
    <div style="width:100%">
                ...
                <div class='form-group'>
                    ...
                    <select class="select-pad" id="project_id" name="project_id" disabled>
                        @foreach($projects as $project)
                            <option value="{{$project->id_code}}" {{ ($project->id_code == \Vinkla\Hashids\Facades\Hashids::encode($integration->project_id)) ? 'selected' : '' }}>{{$project->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="row">
            ...
            <div class='form-group col-12'>
                ...
                <div class="d-flex input-group">
                    <input type="text" class="input-pad addon" name="value" id="value" placeholder="Digite o valor" value='{{isset($integration->value) ? $integration->value : ''}}'>
                </div>
            </div>
        </div>
        <div class="row mt-20">
