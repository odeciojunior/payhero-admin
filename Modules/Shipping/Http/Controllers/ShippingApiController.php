...
class ShippingApiController extends Controller
{
    ...
    public function store(ShippingStoreRequest $request, $projectId)
    {
        try {
            ...
            if ($shippingValidated) {
                ...
                if (Gate::allows("edit", [$project])) {
                    if (
                        $shippingValidated["value"] == null ||
                        preg_replace("/[^0-9]/", "", $shippingValidated["value"]) == 0
                    ) {
                    ...
                    $shippingValidated["type_enum"] = $shippingModel
                        ->present()
                        ->getTypeEnum($shippingValidated["type"]);
                    ...
                    $notApplyPlanArray = [];
                    if (
                        isset($shippingValidated["not_apply_on_plans"]) &&
                        !in_array("all", $shippingValidated["not_apply_on_plans"])
                    ) {
                    ...
                    if ($shippingCreated) {
                        CacheService::forgetContainsUnique(
                            CacheService::SHIPPING_RULES,
                            $shippingValidated["project_id"]
                        );
        ...
        try {
            ...
            if (isset($requestValidated) && isset($projectId) && isset($id)) {
                ...
                if (Gate::allows("edit", [$project])) {
                    ...
                    // }
                    if (
                        $requestValidated["value"] == null ||
                        preg_replace("/[^0-9]/", "", $requestValidated["value"]) == 0
                    ) {
                    ...
                    $notApplyPlanArray = [];
                    if (
                        isset($requestValidated["not_apply_on_plans"]) &&
                        !in_array("all", $requestValidated["not_apply_on_plans"])
                    ) {
                    ...
                    if (
                        !$this->verifyAllPlansHasActiveShippings(
                            $shipping,
                            array_diff($applyPlanArray, $notApplyPlanArray),
                            $requestValidated["status"]
                        )
                    ) {
                        return response()->json(
                            ["message" => "Impossível editar, existem planos que ficarão sem nenhum frete vinculado!"],
                            400
                        );
                    }
                    ...
                    if (!$requestValidated["pre_selected"] && !$shipping->pre_selected) {
                        $sp = $shippingModel
                            ->where([["project_id", $shipping->project_id], ["pre_selected", 1]])
                            ->get();
                    ...
                    if ($shippingUpdated) {
                        $shippings = $shippingModel
                            ->where([["project_id", $shipping->project_id], ["status", 1]])
                            ->get();
        ...
        try {
            if (isset($id) && !empty($id)) {
                ...
                if ($shipping->sales_count > 0) {
                    return response()->json(
                        ["message" => "Impossível excluir, existem vendas associados a este frete!"],
                        400
                    );
                }
                ...
                if ($shipping) {
                    ...
                    if ($project->shippings_count == 1) {
                        return response()->json(
                            ["message" => "Impossivel excluir, o projeto deve ter no minímo 1 frete cadastrado!"],
                            400
                        );
                    }
                    ...
                    if (!$this->verifyAllPlansHasActiveShippings($shipping)) {
                        return response()->json(
                            ["message" => "Impossível excluir, existem planos que ficarão sem nenhum frete vinculado!"],
                            400
                        );
                    }
                    ...
                    if (Gate::allows("edit", [$project])) {
                        if ($shipping->pre_selected) {
                            $shippingPreSelected = $shippingModel
                                ->where([["project_id", $shipping->project_id], ["id", "!=", $shipping->id]])
                                ->first();
    ...
    private function verifyAllPlansHasActiveShippings(
        $shipping,
        array $currentApplyOnPlans = [],
        $active_flag = false
    ): bool {
        $where = [["project_id", $shipping->project_id], ["status", true]];
        ...
        foreach ($shipping->project->plans as $plan) {
            ...
            foreach ($otherShippings as $otherShipping) {
                ...
                if (
                    $shipping->id == $otherShipping->id &&
                    !empty($currentApplyOnPlans) &&
                    $active_flag &&
                    ((isset($currentApplyOnPlans[0]) && $currentApplyOnPlans[0] == "all") ||
                        in_array($plan->id, $currentApplyOnPlans))
                ) {
