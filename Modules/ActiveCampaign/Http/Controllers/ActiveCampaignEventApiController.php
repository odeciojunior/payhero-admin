...
class ActiveCampaignEventApiController extends Controller
{
    ...
    public function show(Request $request)
    {
        try {
            ...
            activity()
                ->on($activecampaignEventModel)
                ->tap(function (Activity $activity) use ($id, $event) {
                    $activity->log_name = "visualization";
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (!empty($integrationId)) {
                $event = $activecampaignEventModel
                    ->where("activecampaign_integration_id", $integrationId)
                    ->where("event_sale", $data["events"])
                    ->first();
                if ($event) {
        ...
        try {
            if (!empty($id)) {
                ...
                if ($event) {
                    ...
                    if (
                        $activeCampaignService->setAccess(
                            $integration->api_url,
                            $integration->api_key,
                            $integration->id
                        )
            ...
            Log::warning(
                "Erro ao tentar acessar tela editar Evento ActiveCampaign (ActiveCampaignEventController - edit)"
            );
            report($e);
