        ...
        try {
            if (!empty($request->input("email")) && !empty($request->input("company"))) {
                    ...
                    try {
                        ...
                        if (!$companyService->isDocumentValidated($company)) {
                            return response()->json(
                                [
                                    "message" =>
                                        "Envio de convites indisponível, os documentos da empresa precisam estar aprovados!",
                                ],
                                400
                            );
                        }
                        ...
                        if (empty($user)) {
                            $emailInvited = $emailService->sendInvite(
                                $request->input("email"),
                                Hashids::encode($company)
                            );
                        } else {
                            return response()->json(
                                [
                ...
                return response()->json(
                    [
                        "message" =>
                            "Erro ao tentar enviar convite, Email e Empresa a receber são campos obrigatórios.",
                    ],
                    400
                );
            }
        ...
        try {
            ...
            if ($invitationId) {
                ...
                if (FoxUtils::validateEmail($invitation->email_invited) && !empty($invitation->company_id)) {
                    ...
                    $emailInvited = $sendgridService->sendInvite(
                        $invitation->email_invited,
                        Hashids::encode($invitation->company_id)
                    );
                    if ($emailInvited == "error") {
                        return response()->json(
                            [
                    ...
                    } elseif ($emailInvited->statusCode() != 202) {
                        return response()->json(
                            [
                                "message" => "Erro ao tentar reenviar convite, tente novamente mais tarde.",
                            ],
