...
//                    }
                    ...
                    if ($sale_id && empty($sale_contestation)) {
                        ...
                        $sale_contestation->transaction_date = isset($array_with_headers['Data da Transação']) ?
                            \Carbon\Carbon::createFromFormat("dmY", $array_with_headers['Data da Transação']) : null;
                        $sale_contestation->expiration_date = isset($array_with_headers['Data do Retorno']) ?
                            \Carbon\Carbon::createFromFormat("dmY", $array_with_headers['Data do Retorno']) : null;
                        $sale_contestation->request_date = isset($array_with_headers['Data da Solicitação']) ?
                            \Carbon\Carbon::createFromFormat("dmY", $array_with_headers['Data da Solicitação']) : null;
                        $sale_contestation->reason = isset($array_with_headers['Motivo do Chargeback']) ? $array_with_headers['Motivo do Chargeback'] : null;
                        $sale_contestation->save();

                    } else if (!empty($sale_contestation)) {
