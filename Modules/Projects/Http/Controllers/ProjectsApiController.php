                        ...
                        if ($photo != null) {
                            try {
                                ...
                                $digitalOceanPath = $digitalOceanService
                                    ->uploadFile("uploads/user/" . Hashids::encode(auth()->user()->account_owner) . '/public/projects/' . Hashids::encode($project->id) . '/main', $photo);
    ...
    public function edit($id)
    {
        try {
            if (isset($id)) {
                ...
                $userProject = $userProjectModel->where('user_id', $user->account_owner)
                                                ->where('project_id', $idProject)->first();
                $userProject = new UserProjectResource($userProject);

                $shopifyIntegrations = $shopifyIntegrationModel->where('user_id', $user->account_owner)
                    ...
                    if ($projectUpdate) {
                        try {
                            ...
                            if ($projectPhoto != null) {
                                ...
                                $digitalOceanPath = $digitalOceanService
                                    ->uploadFile('uploads/user/' . Hashids::encode(auth()->user()->account_owner) . '/public/projects/' . Hashids::encode($project->id) . '/main', $projectPhoto);
                                $project->update([
                            ...
                            if ($projectLogo != null) {
                                ...
                                $digitalOceanPathLogo = $digitalOceanService
                                    ->uploadFile('uploads/user/' . Hashids::encode(auth()->user()->account_owner) . '/public/projects/' . Hashids::encode($project->id) . '/logo', $projectLogo);
