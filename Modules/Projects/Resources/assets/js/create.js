            ...
            if (!isEmpty(response)) {
                ...
                $.each(response, (key, company) => {
                    ...
                    if (company.company_type == "physical person") {
                        if (
                            company.user_address_document_status != "approved" ||
                            company.user_personal_document_status != "approved"
                        ) {
                            dataSelect = `<option value=${company.id} ${
                                company.active_flag == 0 ? "disabled" : ""
                            } disabled>${company.name}</option>`;
                        } else {
                            dataSelect = `<option value=${company.id} ${company.active_flag == 0 ? "disabled" : ""}>${
                                company.name
                            }</option>`;
                        }
