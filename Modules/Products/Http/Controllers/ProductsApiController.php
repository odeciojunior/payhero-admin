...
class ProductsApiController extends Controller
{
    ...
    public function index(IndexProductRequest $request)
    {
        try {
            ...
            $productsSearch = $productsModel->where('user_id', auth()->user()->account_owner);
    ...
    public function store(CreateProductRequest $request)
    {
        try {
            ...
            $data['shopify']            = 0;
            $data['user']               = auth()->user()->account_owner;
            $data['price']              = preg_replace("/[^0-9]/", "", $data['price']);
            $data['cost']               = preg_replace("/[^0-9]/", "", $data['cost']);
            $data['user_id']            = auth()->user()->account_owner;
            ...
            if ($productPhoto != null) {
                ...
                try {
                    ...
                    $digitalOceanPath = $this->getDigitalOceanFileService()
                                             ->uploadFile('uploads/user/' . Hashids::encode(auth()->user()->account_owner) . '/public/products', $productPhoto);
