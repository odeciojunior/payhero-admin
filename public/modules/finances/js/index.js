    ...
    function getCompanies() {
        ...
        $.ajax({
            ...
            success: (response) => {
                ...
                let companiesDataHtml = '';
                $(response.data).each(function (index, value) {
                    if (value.capture_transaction_enabled) {
                        itsApprovedTransactGetnet = true;
                        companiesDataHtml += `<option country="${value.country}" value="${value.id}">${value.name}</option>`;
                    }
                });
                $("#statement_company_select").append(companiesDataHtml);
                $("#transfers_company_select").append(companiesDataHtml);
                $("#transfers_company_select_mobile").append(companiesDataHtml);
