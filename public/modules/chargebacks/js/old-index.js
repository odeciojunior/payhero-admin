$(document).ready(function () {
    ...
    function updateChagebacksTable(link = null) {
        ...
        if (link == null) {
            link = '/old-chargebacks/getchargebacks?' + 'fantasy_name=' + $("#fantasy_name").val() + '&user_name=' + $("#user_name").val() + '&date=' + $('#date_range').val();
        } else {
            link = '/old-chargebacks/getchargebacks' + link + '&fantasy_name=' + $("#fantasy_name").val() + '&user_name=' + $("#user_name").val() + '&date=' + $('#date_range').val();
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $.each(response.data, function (index, value) {
                    ...
                    dados += "<td>" + value.chargeback_tax + ' (' + (value.count_sales_chargeback) + ' de ' + value.count_sales_approved + ' aprovadas' + ')' + "</td>";
                    dados += "<td>";
                    dados += "<div><a role='button' class='chargeback_details pointer' company='" + value.id + "' data-target='#details-modal' data-toggle='modal' title='Visualizar' style='margin-right:10px;'><i class='material-icons gradient'>remove_red_eye</i></button></a></div>";
                    dados += "</div></td>";
                    dados += '</tr>';
    ...
    function getTotalValues() {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#total-chargeback-tax').html(response.total_chargeback_tax + ' (' + response.total_chargeback + ' de ' + response.total_credit_card_approved + ' aprovadas' + ')');
            }
        });
    }
    function datePicker() {
