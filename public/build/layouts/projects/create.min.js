...
$('#company-navbar').change(function () {
  ...
  updateCompanyDefault().done(function (data1) {
    getCompaniesAndProjects().done(function (data2) {
      console.log(data2); // $('.company_name').val( data2.company_default_name );// $('.company_name').val( sessionStorage.getItem('company_default_name') );
...
$(document).ready(function () {
  ...
  getCompaniesAndProjects().done(function (data) {
    $('.company_name').val(data.company_default_name); //     $('.company_id').val( data.company_default );//sessionStorage.getItem('company_default') );
        ...
        $.each(response, function (key, company) {
          ...
          if (company.company_type == 'physical person') {
            if (company.user_address_document_status != 'approved' || company.user_personal_document_status != 'approved') {
              dataSelect = "<option value=".concat(company.id, " ").concat(company.active_flag == 0 ? 'disabled' : '', " disabled>").concat(company.name, "</option>");
            } else {
              dataSelect = "<option value=".concat(company.id, " ").concat(company.active_flag == 0 ? 'disabled' : '', ">").concat(company.name, "</option>");
            }
  ...
  function verifyFields() {
    if ($('#name').val().length === 0) {
      ...
      return true; // } else if ($("#company option:selected").val().length === 0) {
  ...
  btnSave.on('click', function () {
    if (verifyFields()) {
      ...
      loadingOnScreen();
      var formData = new FormData(document.querySelector('#form-create-project'));
