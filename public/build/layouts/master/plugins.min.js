...
$(document).ready(function () {
  ...
  $('#company-navbar').change(function () {
    var thisPage = new URL(window.location);
    caminho = thisPage.pathname;
    if (caminho.match(/\/projects\/[a-zA-Z0-9]*/i)) return;
    loadingOnScreen();
    $.ajax({
      method: 'POST',
  ...
  $("input[name='step-2-other-ecommerce-check']").change(function () {
    var input = $("input[name='step-2-other-ecommerce']");
  ...
  $("input[name='step-2-know-cloudfox-check']").change(function () {
    var input = $("input[name='step-2-know-cloudfox']");
  ...
  $.ajax({
    ...
    success: function success(response) {
      if (response.data.account !== 'approved') {
        ...
        if (_verifyAccount.account !== 'approved') {
          localStorage.setItem('verifyAccount', JSON.stringify(response.data));
        }
      }
    }
...
function selectCompanies() {
  var parseSessionStorageCompanies = JSON.parse(sessionStorage.getItem('companies'));
  $('#company-navbar').append('<option value="v2RmA83EbZPVpYB">Empresa Demo</option>');

  for (var i = 0; i < parseSessionStorageCompanies.length; i++) {
    if (sessionStorage.getItem('company_default') === parseSessionStorageCompanies[i].id) itemSelected = 'selected="selected"';else itemSelected = '';
    if (parseSessionStorageCompanies[i].active_flag == true) itemDisabled = '';else itemDisabled = 'disabled="disabled"';
    ...
    if (parseSessionStorageCompanies[i].company_type == '1') {
      ...
      if (parseSessionStorageCompanies[i].name.length > 20) companyName = parseSessionStorageCompanies[i].name.substring(0, 20) + '...';else companyName = parseSessionStorageCompanies[i].name;
...
function getCompanies() {
  var thisPage = window.location.hostname;
