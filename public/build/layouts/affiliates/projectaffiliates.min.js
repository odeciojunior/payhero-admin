...
$(document).ready(function () {
  ...
  function getProjects() {
    ...
    if (loading == 'y') loadingOnScreen();else loadOnAny('.page-content');
        ...
        if (!isEmpty(response.data)) {
          ...
          $.each(response.data, function (i, project) {
            if (project.affiliated == false) {
              $("#project-affiliate").append($("<option>", {
                value: project.id,
                text: project.name
              }));
              $("#project-affiliate-request").append($("<option>", {
                value: project.id,
                text: project.name
              }));
            }
  ...
  function getAffiliates() {
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        if (response.data == "") {
          $("#body-table-affiliates").html("<tr class='text-center'><td colspan='8' style='height: 257px; vertical-align: middle;'><img style='width:124px;margin-right:12px;' src='" + $("#body-table-affiliates").attr("img-empty") + "'> Nenhum afiliado encontrado</td></tr>");
        } else {
          $.each(response.data, function (index, value) {
            if (value.percentage == "" || value.percentage == "0") value.percentage = "0%";
            data = "";
            data += "<tr>";
            data += '<td class="" style="vertical-align: middle;">' + value.name + "</td>"; // data += '<td class="" style="vertical-align: middle;">' + value.email + '</td>';
            ...
            data += "<a title='Visualizar' class='mg-responsive pointer details-affiliate' affiliate='" + value.id + "'><span class='o-eye-1'></span></a>";
            data += "<a title='Editar' class='mg-responsive pointer edit-affiliate'    affiliate='" + value.id + "'><span class='o-edit-1'></span></a>";
            data += "<a title='Excluir' class='mg-responsive pointer delete-affiliate' affiliate='" + value.id + "'><span class='o-bin-1'></span></a>";
            data += "</td>";
            data += "</tr>";
        ...
        $(document).on("click", ".edit-affiliate", function () {
          ...
          $.ajax({
            ...
            success: function success(response) {
              if (response.data.percentage == "" || response.data.percentage == "0") response.data.percentage = "0%";
              $("#modal-edit-affiliate .affiliate-id").val(affiliate);
              $("#modal-edit-affiliate .affiliate-name").val(response.data.name); // $('#modal-edit-affiliate .affiliate-email').val(response.data.email);
        ...
        $("#modal-edit-affiliate .btn-update").on("click", function () {
          var formData = new FormData(document.getElementById("form-update-affiliate")); //console.log(formData);
          // formData.append("integration_id", integrationId);

          var affiliate = $("#modal-edit-affiliate .affiliate-id").val();
          loadingOnScreen();
          $.ajax({
            method: "POST",
            url: "/api/affiliates/" + affiliate,
  ...
  function getAffiliatesRequest() {
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        if (response.data == "") {
          $("#body-table-affiliate-requests").html("<tr class='text-center'><td colspan='8' style='height: 257px; vertical-align: middle;'><img style='width:124px;margin-right:12px;' src='" + $("#body-table-affiliate-requests").attr("img-empty") + "'>Nenhuma solicitação de afiliação encontrada</td></tr>");
        } else {
          $.each(response.data, function (index, value) {
            data = "";
            data += "<tr>";
            ...
            data += '<td class="" style="vertical-align: middle;">' + value.project_name + "</td>";
            data += '<td class="" style="vertical-align: middle;">' + value.date + "</td>"; // data += '<td class="text-center" ><span class="badge badge-' + badgeAffiliateRequest[value.status] + '">' + value.status_translated + '</span></td>';

            data += "<td class='text-center text-nowrap'>";

            if (value.status != 3 && verifyAccountFrozen() == false) {
              data += "<a title='Aprovar' class='text-white ml-2 mb-1 mt-1 badge badge-success pointer evaluate-affiliate' affiliate='" + value.id + "' status='3'>Aprovar</a>";

              if (value.status != 4) {
                data += "<a title='Recusar' class='text-white ml-2 mb-1 mt-1 badge badge-danger pointer evaluate-affiliate' affiliate='" + value.id + "' status='4'>Recusar</a>";
              }
            }
