    ...
    $.ajax({
      ...
      $.each(data, function (c, company) {
        $.each(company.projects, function (i, project) {
          if (dataSales.includes(project.id)) $("#select_projects").append($("<option>", {
            value: project.id,
            text: project.name
          }));
        });
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        if (response.totalPaidValueAproved == 'R$ 0,00' || response.totalPaidValueAproved == false || !response.totalPaidValueAproved) {
          ...
          var split = response.totalPaidValueAproved.split(/\s/g);
        ...
        $("#percent-credit-card").html("\n                    ".concat(parseFloat(response.totalPercentCartao).toFixed(1), " %\n                "));
        $("#percent-values-boleto").html("\n                    ".concat(parseFloat(response.totalPercentPaidBoleto).toFixed(1), " %\n                "));
        $("#percent-values-pix").html("\n                    ".concat(parseFloat(response.totalPercentPaidPix).toFixed(1), " %\n                "));
        ...
        $("#percent-boleto-convert").html("\n                    <span class=\"money-td\"> ".concat(parseFloat(response.convercaoBoleto).toFixed(1), " % </span>\n                "));
        $("#percent-credit-card-convert").html("\n                    <span class=\"money-td\"> ".concat(parseFloat(response.convercaoCreditCard).toFixed(1), " % </span>\n                "));
        $("#percent-pix-convert").html("\n                    <span class=\"money-td\"> ".concat(parseFloat(response.convercaoPix).toFixed(1), " % </span>\n                "));
        $("#percent-desktop").html("\n                    ".concat(parseFloat(response.conversaoDesktop).toFixed(1), " %\n                "));
        $("#percent-mobile").html("\n                    ".concat(parseFloat(response.conversaoMobile).toFixed(1), " %\n                "));
        ...
        if (!isEmpty(response.plans)) {
          ...
          table_data_itens += "<tr class='text-center'><td colspan='3' style='vertical-align: middle'><img style='height:90px' src='" + $("#origins-table-itens").attr("img-empty") + "'>Nenhuma venda encontrada</td></tr>";
        }
    ...
    $.ajax({
      ...
      success: function success(response) {
        if (response.data.length == 0) {
          $("#origins-table").html("<td colspan='3' class='text-center'><img style='height:90px' src='" + $("#origins-table-itens").attr("img-empty") + "'><br> Nenhuma venda encontrada</div>");
          $("#pagination").html("");
        } else {
          var table_data = "";
