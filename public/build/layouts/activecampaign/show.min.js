...
$(function () {
  var projectId = $(window.location.pathname.split("/")).get(-1); // COMPORTAMENTOS DA TELA

  $("#tab_configuration").click(function () {
    show();
  }); // FIM - COMPORTAMENTOS DA TELA

  show(); //carrega detalhes da loja
  ...
  function create() {
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        $("#btn-modal").html('<i class="material-icons btn-fix"> save </i>Salvar'); // $("#modal_add_event").modal('show');

        $("#btn-modal").removeClass("d-none");
        $("#form-update-event").hide();
        $("#form-register-event").show(); // $(document).on('click', '.btnDelete', function (event) {
        //     event.preventDefault();
    ...
    $.ajax({
      ...
      success: function success(response) {
        if (isEmpty(response.data)) {
          ...
          var hasPagination = $("#pagination-events").children().length > 0;
          ...
          $.each(response.data, function (index, value) {
            ...
            data += "</td>";
            var addList = value.add_list != null ? value.add_list.list : "";
            var rmList = value.remove_list != null ? value.remove_list.list : "";
            ...
            data += "<a title='Visualizar' class='mg-responsive pointer details-event' event='" + value.id + "' role='button'><span class=''><img src='/build/global/img/icon-eye.svg'/></span></a>";
            data += "<a title='Editar' class='mg-responsive pointer edit-event' event='" + value.id + "' role='button'data-toggle='modal' data-target='#modal-content'><span class=''><img src='/build/global/img/pencil-icon.svg'/></span></a>";
            data += "<a title='Excluir' class='mg-responsive pointer delete-event' event='" + value.id + "' role='button'data-toggle='modal' data-target='#modal-delete'><span class=''><img src='/build/global/img/icon-trash-tale.svg'/></span></a>";
            data += "</td>";
            data += "</tr>";
          ...
          $.ajax({
            ...
            success: function success(response) {
              if (response.message == "error") {
                ...
                var addList = response.data.add_list != null ? response.data.add_list.list : "";
                var rmList = response.data.remove_list != null ? response.data.remove_list.list : "";
          ...
          $.ajax({
            ...
            success: function success(response) {
              ...
              if (lists !== null) {
                $(lists.lists).each(function (index, data) {
                  var selected = "";
                  var idAddList = getIdAddList(response);
                ...
                $(lists.lists).each(function (index, data) {
                  var selected = "";
                  var idRemoveList = getIdRemoveList(response);
...
function getIdAddList(response) {
  try {
    var id = response.event.add_list.id;
    return id;
  } catch (e) {
    return 0;
  }
...
function getIdRemoveList(response) {
  try {
    var id = response.event.remove_list.id;
    return id;
  } catch (e) {
    return 0;
  }
