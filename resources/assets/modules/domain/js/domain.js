...
$(document).ready(function () {
    ...
    function updateDomains(link = null) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.data == "") {
                    $("#domain-table-body").html(
                        "<tr class='text-center'><td colspan='4' style='height: 70px; vertical-align: middle;'>Nenhum dominio encontrado</td></tr>"
                    );
                } else {
    ...
    function modalUpdateDomains(index, value) {
        ...
        dados +=
            ...
            "<a role='button' class='details-domain pointer mr-30' status='" +
            value.status +
            "' domain='" +
            value.id +
            "' data-target='#modal-content' data-toggle='modal'><span class='o-eye-1'></span> </a>" +
            "<a role='button' class='edit-domain pointer' status='" +
            value.status +
            "' domain='" +
            value.id +
            "' data-target='#modal-content' data-toggle='modal'><span class='o-edit-1'></span> </a>" +
            "<a role='button' class='delete-domain pointer ml-30' domain='" +
            value.id +
        ...
        $("#modal-add-body")
            .append(
                '<div class="swal2-icon swal2-info swal2-animate-info-icon" style="display: flex;">i</div>' +
                    ...
                    '" style="font-size: 25px">Verificar</button>' +
                    "</div>"
            )
            .show("slow");
            ...
            if ($("#nome_registro").val() != "" && $("#valor_registro").val() != "") {
                ...
                $("#new_registers").after(
                    "<tr class='alert-info' data-row='" +
                        ...
                        $("#new_registers_table tr").length +
                        "' class='btn btn-primary remove-record' onclick='deleteRow(this)'>Remover</button>" +
                        "</td></tr>"
                );
                $("#nome_registro").val("");
                $("#valor_registro").val("");
