$(document).ready(function () {
                    ...
                    for (error in response.responseJSON.errors) {
                        switch (String(response.responseJSON.errors[error])) {
                            case "The profile photo must be a file of type: jpeg, jpg, png.":
                                alertCustom(
                                    "error",
                                    "A imagem deve estar em um dos seguintes formatos: jpeg, jpg, png."
                                );
                                break;
                            default:
    ...
    $("#new_password").on("input", function () {
        if (
            ...
            $("#new_password_confirm").val().length > 5 &&
            $("#new_password").val() == $("#new_password_confirm").val()
        ) {
...
Dropzone.options.dropzoneDocuments = {
    ...
    success: function (file, response) {
        ...
        if (response.personal_document_translate == "Em análise") {
            $("#personal_document_badge")
                .removeAttr("class")
                .attr("class", "badge badge-pendente")
                .text(response.personal_document_translate);
        }
        if (response.address_document_translate == "Em análise") {
            $("#address_document_badge")
                .removeAttr("class")
                .attr("class", "badge badge-pendente")
                .text(response.address_document_translate);
        }
        swal({
            position: "bottom",
            type: "success",
