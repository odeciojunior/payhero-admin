    ...
    public function down()
    {
        Schema::table("projects", function (Blueprint $table) {
            $table
                ...
                ->nullable()
                ->after("woocommerce_id");
            $table
                ->string("installments_interest_free")
                ->nullable()
                ->after("installments_amount");
            $table->string("invoice_description")->nullable();
            $table
                ->string("contact")
                ->nullable()
                ->index("projects_contact_IDX");
            $table
                ->boolean("contact_verified")
                ->default(0)
                ->index("projects_contact_verified_IDX");
            $table
                ->string("logo")
                ->nullable()
                ->after("contact_verified");
            $table
                ->boolean("boleto")
                ->default(1)
                ->after("logo");
            ...
            $table
                ...
                ->nullable()
                ->index("projects_support_phone_IDX")
                ->after("analyzing_redirect");
            $table
                ->boolean("support_phone_verified")
                ->default(0)
                ->index("projects_support_phone_verified_IDX")
                ->after("support_phone");
            $table
            ...
            $table
                ->string("countdown_timer_description")
                ->nullable()
                ->after("countdown_timer_time");
            $table
                ->string("countdown_timer_finished_message")
            ...
            $table
                ->json("custom_message_configs")
                ->nullable()
                ->after("checkout_notification_configs");
            $table
                ->integer("checkout_type")
                ->default(1)
                ->after("updated_at");
            $table
                ->integer("pre_selected_installment")
                ->default(12)
                ->after("checkout_type");
            $table
                ->boolean("required_email_checkout")
                ->default(0)
                ->after("pre_selected_installment");
            $table
                ->unsignedInteger("document_type_checkout")
                ->default(3)
                ->after("required_email_checkout");
            $table
                ->boolean("product_amount_selector")
                ->default(1)
                ->after("document_type_checkout");
        });
    }
}
