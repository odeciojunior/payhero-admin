...
// die;
    ...
    public function detalhesSaldoFuturo(Request $request){
        ...
        foreach($transactionPayables as $transactionPayable){
            ...
            $data_pagamento = date('d/m/Y',strtotime($transactionPayable->payment_date));
            if($data_pagamento >= $hoje){
                $lancamentos_futuros[] = $data_pagamento;
            }

        }
dd($lancamentos_futuros);
        return response()->json($transactionPayables);
