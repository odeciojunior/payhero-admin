...
{
    public function getAllCloudFlareId()
    {
        $domainModel       = new Domain();
        $domainRecordModel = new DomainRecord();
        $cf                = new CloudFlareService();

        $domains = $domainModel->with('records')->get();

        foreach ($domains as $domain) {

            try {
                $records = $cf->getRecords($domain->name);
            } catch (\Exception $e) {

            }
            foreach ($domain->records as $domainRecord) {
                if (strpos($domainRecord->name, $domain->name) === false) {

                    $dm = $domainRecord->name . '.' . $domain->name;
                } else {
                    $dm = $domainRecord->name;
                }
            ...
            /*
                        foreach ($records as $record) {
                            ...
                            $domainRecord = $domainRecordModel->where('type', $record->type)
...
";
        ...
        foreach ($tokens[0] as $key => $item) {
            ...
            $html = preg_replace($from, 'fox-fox-fox', $html, 1);
        }
        ...
        //dd($cf->deleteZone('cloudteste2.tk'));

        //dd($cf->getRecords('cloudteste2.tk'));
        /*
