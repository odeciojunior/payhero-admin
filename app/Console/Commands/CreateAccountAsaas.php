        ...
        try {
            ...
            $data = [
                ...
                "email" => foxutils()->isProduction()
                    ? $company->user->email
                    : "cloudfox-teste" . $company->user->email,
                "cpfCnpj" => foxutils()->onlyNumbers($company->document),
                "phone" => foxutils()->isProduction()
                    ? $companyPhoneNumber["dd"] . " " . $companyPhoneNumber["number"]
                    : "55 995555555",
    ...
    private function updateToReviewStatus($result, $company): array
    {
        try {
            if ($result->status == "error") {
                throw new Exception(
                    "Empresa {$company->id} - {$company->fantasy_name}, erro: " . $result->errors[0]->description
                );
            }
