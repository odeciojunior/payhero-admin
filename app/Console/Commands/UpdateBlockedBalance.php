    ...
    public function handle()
    {
        try {
            ...
            $transactions = $transactionModel
                ->with(["sale.tracking", "sale.productsPlansSale"])
                ->join("sales", "sales.id", "transactions.sale_id")
                ->whereNull("invitation_id")
                ->where(function ($queryStatus) use ($transactionModel, $salesModel) {
                    $queryStatus
                        ->where(function ($transfered) use ($transactionModel) {
                            $transfered->where(
            ...
            foreach ($transactions->cursor() as $transaction) {
                ...
                if ($sale->status == 24) {
                    $created = $this->createBlockReasonSale(
                        $sale->id,
                        $blockReasonModel->present()->getReasonEnum("in_dispute")
                    );
                }
                if ($sale->tracking->count() < $sale->productsPlansSale->count()) {
                    $created = $this->createBlockReasonSale(
                        $sale->id,
                        $blockReasonModel->present()->getReasonEnum("without_tracking")
                    );
                }
                if (
                    $sale->tracking
                        ->where("system_status_enum", (new Tracking())->present()->getSystemStatusEnum("duplicated"))
                        ->count()
                ) {
                    $created = $this->createBlockReasonSale(
                        $sale->id,
                        $blockReasonModel->present()->getReasonEnum("duplicated")
                    );
                }
                if (
                    $sale->tracking
                        ->where(
                            "system_status_enum",
                            (new Tracking())->present()->getSystemStatusEnum("no_tracking_info")
                        )
                        ->count()
                ) {
                    $created = $this->createBlockReasonSale(
                        $sale->id,
                        $blockReasonModel->present()->getReasonEnum("no_tracking_info")
                    );
                }
                if (
                    $sale->tracking
                        ->where(
                            "system_status_enum",
                            (new Tracking())->present()->getSystemStatusEnum("unknown_carrier")
                        )
                        ->count()
                ) {
                    $created = $this->createBlockReasonSale(
                        $sale->id,
                        $blockReasonModel->present()->getReasonEnum("unknown_carrier")
                    );
                }
                if (
                    $sale->tracking
                        ->where(
                            "system_status_enum",
                            (new Tracking())->present()->getSystemStatusEnum("posted_before_sale")
                        )
                        ->count()
                ) {
                    $created = $this->createBlockReasonSale(
                        $sale->id,
                        $blockReasonModel->present()->getReasonEnum("posted_before_sale")
                    );
                }
                if (empty($created)) {
