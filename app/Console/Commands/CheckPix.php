...
class CheckPix extends Command
{
    ...
    {
        try {
            $sales = Sale::where([["payment_method", "=", Sale::PIX_PAYMENT], ["status", "=", Sale::STATUS_CANCELED]])
            ...
            foreach ($sales as $sale) {
                ...
                if ($responseCheckout->status == "success" and $responseCheckout->payment == true) {
                    $saleModel = Sale::where([
                        ...
                        ->whereDate("start_date", Carbon::parse($sale->start_date)->format("Y-m-d"))
                        ->first();

                    if (empty($saleModel)) {
                        report(
                            new Exception(
                                'Local command para checar as venda paga na Gerencianet e com problema no pagamento. $sale->id = ' .
                                    $sale->id .
