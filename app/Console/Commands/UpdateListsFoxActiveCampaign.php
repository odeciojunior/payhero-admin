...
class UpdateListsFoxActiveCampaign extends Command
{
    ...
    protected $description = "Atualiza lista de contatos da Azcend no ActiveCAmpaign";

    private $apiUrlFox = "https://azcend.api-us1.com";
    ...
    private function listUsers100k($listId)
    {
        try {
            $users = User::select(
                ...
                "email_verified",
                DB::raw(
                    "(SELECT SUM(total_paid_value) FROM sales WHERE sales.owner_id = users.id AND status = 1 AND created_at > NOW() - INTERVAL 30 DAY ) as total_value"
                )
            )
                ->havingRaw(
                    "(SELECT SUM(total_paid_value) FROM sales WHERE sales.owner_id = users.id AND status = 1 AND created_at > NOW() - INTERVAL 30 DAY ) > 100000"
                )
                ->get();
            ...
            $users = User::doesntHave("sales")
                ...
                ->whereHas("companies", function ($query) {
                    $query->where(function ($queryCompany) {
                        $queryCompany
                            ->where(function ($companyJuridical) {
                                $companyJuridical
                            ...
                            ->orWhere(function ($companyPhysical) {
                                $companyPhysical->where("company_type", Company::PHYSICAL_PERSON);
                            });
                    });
                })
