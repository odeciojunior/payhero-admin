...
class CheckSales extends Command
{
    ...
    public function getBalanceCloudfox(){
        $totalSalesIn = DB::select("SELECT SUM(value) as total FROM transactions WHERE gateway_id = 21 and status_enum IN (1,2)  AND company_id is not null AND deleted_at is NULL;");
        $totalFoxIn = DB::select("SELECT SUM(value) as total FROM transactions WHERE gateway_id = 21 and status_enum IN (1,2)  AND company_id is null AND deleted_at is NULL;");

        $totalIn = $totalSalesIn['0']->total + $totalFoxIn['0']->total;
        $this->line(str_pad('Total Sale In:',20,' ',STR_PAD_RIGHT).str_pad($totalSalesIn['0']->total,20,' ',STR_PAD_LEFT));
        $this->line(str_pad('Total Fox In:',20,' ',STR_PAD_RIGHT).str_pad($totalFoxIn['0']->total,20,' ',STR_PAD_LEFT));
    ...
    public function verifySaleSafeAutorizado(){
        ...
        foreach($rows as $row)
        {
            $sale = DB::table('sales')->select('id','status','payment_method', 'original_total_paid_value')
                ->where('gateway_id',21)
                ->where('status', '!=', Sale::STATUS_APPROVED)
                ->where('id', $row->SaleId)->first();
    ...
    public function verifySaleSafeDevolvido(){
        ...
        $sales= Sale::select('id','status','payment_method', 'original_total_paid_value')->whereIn('id', $saleSafe2pay)->get()->toArray();

        foreach($sales as $key=>$sale) {
            if(!in_array($sale['status'], [Sale::STATUS_REFUNDED, Sale::STATUS_CANCELED_ANTIFRAUD, Sale::STATUS_SYSTEM_ERROR] )) {
    ...
    public function verifySaleSafePendingByIdTransaction($syncSales = null)
    {
        ...
        foreach($rows as $row) {
            ...
            $bar->advance();
            if(empty($row->IdTransaction)) $aux2++;//

            $sales = Sale::select('id','status', 'gateway_transaction_id')
            ->where('gateway_id', Gateway::SAFE2PAY_PRODUCTION_ID)->where('gateway_transaction_id', $row->IdTransaction)->first();
            if(!$sales) {
                $aux2++;
                Log::info(json_encode([
            ...
            $sales = Sale::select('id','status', 'gateway_transaction_id')
            ->where('gateway_id', Gateway::SAFE2PAY_PRODUCTION_ID)->whereIn('gateway_transaction_id', $saleSafe2pay);
    ...
    public function verifySaleCloudfoxPendingByIdTransaction()
    {
        ...
        $sales= Sale::select('id','status', 'gateway_transaction_id')->where('gateway_id', Gateway::SAFE2PAY_PRODUCTION_ID)
    ...
    public function verifySaleSafeDevolvidoByIdTransaction()
    {
        ...
        $sales= Sale::select('id','status','payment_method', 'original_total_paid_value', 'gateway_transaction_id')->where('gateway_id', Gateway::SAFE2PAY_PRODUCTION_ID)->whereIn('gateway_transaction_id', $saleSafe2pay);
    ...
    public function verifySaleSafeChargebackByIdTransaction(){
        ...
        $sales= Sale::select('id','status','payment_method', 'original_total_paid_value', 'gateway_transaction_id')->whereIn('gateway_transaction_id', $saleSafe2pay)->get()->toArray();
    ...
    public function deleteOrCreateLog($file = null) {
        if (file_exists($this->fileLog) and !$file) file_put_contents($this->fileLog, null);
    ...
    public function verifySaleSafe_old(){
        ...
        $sales = DB::table('sales')->select('id','status','payment_method')
        ->where('gateway_id',21)
    ...
    */
    ...
    public function verifySale(){
        ...
        foreach($rows as $row){
            ...
            $ids[] = $saleId;
            if(count($ids)==500){
                $sales = DB::table('sales')->select('id','status')->whereIn('id',$ids)->get();
    ...
    {
        $safe = new Safe2payGateway();
        $sales = DB::table('sales')->select('id','gateway_transaction_id')->where('gateway_id',21)->where('status',Sale::STATUS_APPROVED);
    ...
    public function changeSaleFromRefused()
    {
        ...
        ->with('transactions')
        ->whereIn('id', $saleIds)->where('status', Sale::STATUS_REFUNDED)->get(); //->toArray();
    ...
    {
        $saleIds = [ 1502470, 1525818, 1536734, 1537056, 1547459, 1552765, 1556185, 1557894, 1590249, 1590477, 1590480, 1590488, 1590490, 1590491, 1590494, 1590496, 1590497, 1590498, 1590501, 1590502, 1590503, 1590508, 1590510, 1595503, 1598834, 1599197, 1600587, 1602175, 1607892, 1617325, 1620611, 1622466, 1626345, 1626675, 1627169, 1636864, 1637660, 1640532, 1641255, 1641452, 1651850, 1653172, 1653294, 1657175, 1660120, 1662468, 1662618, 1664439, 1665816, 1667529, 1668497, 1669755, 1671881, 1673529, 1674005, 1677411, 1677476, 1681940, 1681950, 1685366, 1694308, 1694664, 1703071, 1715464, 1715529, 1726731, 1732928, 1748799, 1805626, 1805631, 1805643, 1805662, 1805689, 1805752, 1805785, 1805787, 1805872, 1805874, 1805982, 1806076, 1806082, 1806083, 1806126, 1806168, 1808803, 1808877, 1808882, 1808904, 1808971, 1808987, 1809015, 1809030, 1809033, 1810971, 1826963, 1826942, 1826913, 1826365];

        $sales = Sale::whereIn('id', $saleIds)->where('status', Sale::STATUS_REFUNDED)->get(); //->toArray();
    ...
    public function getTransactionsApprovedCloudfox()
    {
        ...
        $sales = DB::table('sales')->select('id','gateway_transaction_id', 'status')->where('gateway_id',21)->where('status',Sale::STATUS_APPROVED);
