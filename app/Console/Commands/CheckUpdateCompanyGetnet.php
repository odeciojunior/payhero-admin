...
class CheckUpdateCompanyGetnet extends Command
{
    ...
    {
        try {
            $companiesCredencial = GatewaysCompaniesCredential::where("gateway_id", Gateway::GETNET_PRODUCTION_ID)
                ->where("gateway_status", GatewaysCompaniesCredential::GATEWAY_STATUS_REVIEW)
                ->get();
            ...
            foreach ($companiesCredencial as $credential) {
                ...
                if (!foxutils()->isEmpty($company)) {
                    if ($company->company_type == Company::PHYSICAL_PERSON) {
                        // physical person
