...
class PipefyUpdateCard extends Command
{
    ...
    public function checkDocumentUser()
    {
            ...
            ->selectRaw("(
                        SELECT count( c.id ) FROM companies AS c
                            WHERE c.user_id = users.id
                                AND (c.address_document_status = ".CompanyDocument::STATUS_REFUSED."
                                OR c.contract_document_status = ".CompanyDocument::STATUS_REFUSED." )
                            GROUP BY c.user_id
                        ) total_companies_refused")
            ->selectRaw("(
                        SELECT count( c.id ) FROM companies AS c
                            WHERE c.user_id = users.id
                                AND (c.address_document_status = ".CompanyDocument::STATUS_APPROVED."
                                OR c.contract_document_status = ".CompanyDocument::STATUS_APPROVED." )
                            GROUP BY c.user_id
        ...
        foreach ($usersDocument->cursor() as $user) {
            if ($user->address_document_status == UserDocument::STATUS_REFUSED ||
                $user->personal_document_status == UserDocument::STATUS_REFUSED ){
                    if (empty($user->total_companies_active)){
                        ...
                        foreach ($user->companies as $companie){
                            if ($companie->address_document_status == CompanyDocument::STATUS_REFUSED
                                && $companie->contract_document_status == CompanyDocument::STATUS_REFUSED){
                                $pipefyService->moveCardToPhase($user, PipefyService::PHASE_REFUSED_DOCUMENT);
                            }
                        }
                    }
            }elseif ($user->account_is_approved == User::STATUS_ACTIVE &&
                ...
                if (!empty($user->companies)){
                    foreach ($user->companies as $companie){
                        if ($companie->address_document_status == CompanyDocument::STATUS_APPROVED
