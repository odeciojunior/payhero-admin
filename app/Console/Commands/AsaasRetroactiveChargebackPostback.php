...
class AsaasRetroactiveChargebackPostback extends Command
{
    ...
    {
        $sales = Sale::where("gateway_id", 8)
            ->where("status", Sale::STATUS_CHARGEBACK)
            ->whereDate("updated_at", now())
            ->doesnthave("contestations")
            ->doesnthave("saleRefundHistory")
            ->get();
        ...
        foreach ($sales as $sale) {
            ...
            foreach ($refundTransactions as $refundTransaction) {
                ...
                $transfers = Transfer::where("transaction_id", $refundTransaction->id)
                ...
                foreach ($transfers as $transfer) {
                    ...
                    Log::info(
                        "Devolvendo " . $transfer->value . " empresa " . $company->id . " - " . $company->fantasy_name
                    );
                    $company->update([
    ...
    {
        $contestations = SaleContestation::where("gateway_id", 8)
        ...
        foreach ($contestations as $contestation) {
            if (!empty($contestation->sale_id)) {
                $request = DB::table("sale_gateway_requests")
                    ->select("id", "gateway_result")
                    ->where("sale_id", $contestation->sale_id)
                    ->where("gateway_result", "like", "%CONFIRMED%CREDIT_CARD%")
                    ->orderBy("id", "DESC")
                    ->first();
                if (!empty($request)) {
    ...
    {
        $postbacks = GatewayPostback::where("gateway_id", 8)
            ->whereHas("sale", function ($qr) {
                $qr->where("status", Sale::STATUS_APPROVED);
            })
            ->where("data", "like", '%"event": "PAYMENT_REFUNDED"%')
    ...
    {
        $postbacks = GatewayPostback::where("gateway_id", 8)
            ->where("data", "like", "%PAYMENT_CHARGEBACK_REQUESTED%")
            ->whereDate("created_at", now())
            ->get();
    ...
    public function createPostbackRetroactive()
    {
        ...
        foreach ($gateway_postbacks as $row) {
            $requestExist = GatewayPostback::where("sale_id", $row["sale_id"])
                ->where("data", "like", "%PAYMENT_CHARGEBACK_REQUESTED%")
                ->exists();
            if ($requestExist) {
                continue;
    ...
    {
        $saleGatewayRequest = SaleGatewayRequest::where("sale_id", $saleId)
            ->where("send_data", "like", "%split%")
            ->orderBy("id", "desc")
            ->first();
    ...
    public function collectChargeback()
    {
        ...
        foreach ($sales as $sale) {
            ...
            if (
                ...
                str_contains($response->data->status, "CHARGEBACK")
            ) {
                Log::info(
